---
source_pdf: docs/pdf/b2tool.pdf
pair: b02
kind: tool
---

# DoView Tool B2 — DoView Strategy/Outcomes Diagrams as 'Shared Thinking Tools' Explainer

> **Pair:** [Question](b02question.md) · Tool (this page)

In 'B' 'strategy' is captured and worked on in one place — a DoView drill-down strategy/outcomes diagram that acts as an external 'shared thinking artifact' or 'shared thinking tool' to reduce the cognitive load of the strategy task.

## Diagram

### A — Strategy scattered across many separate documents and meetings

```mermaid
flowchart TD
    a1["Literature review / R & D"]
    a2["Cabinet/Board paper"]
    a3["Annual plan"]
    a4["Long term plan"]
    a5["Project team presentations"]
    a6["Stakeholder presentations"]
    a7["Meetings with politicians/board"]
    a8["Organization's internal meetings"]
    a9["Meetings with stakeholders"]
```

In 'A' strategy is scattered across many different documents, meetings and presentations.

### B — Strategy captured in one shared thinking tool

```mermaid
flowchart LR
    core["DoView drill-down strategy diagram"]
    b1["Literature review / R & D"]
    b2["Cabinet/Board paper"]
    b3["Initiative plan"]
    b4["Long term plan"]
    b5["Project team presentations"]
    b6["Stakeholder presentations"]
    b7["Meetings with politicians/board"]
    b8["Organization's internal meetings"]
    b9["Meetings with stakeholders"]
    b1 <--> core
    b2 <--> core
    b3 <--> core
    b4 <--> core
    b5 <--> core
    b6 <--> core
    b7 <--> core
    b8 <--> core
    b9 <--> core
```

In 'B' the DoView drill-down strategy diagram sits at the centre and feeds into (and is updated from) every document, presentation and meeting in the organization's strategy work.

---

*Source: DOVIEW PLANNING AND PRACTICAL OUTCOMES THEORY HANDBOOK (2025). DoView Planning.Org. Copyright Dr Paul W Duignan.*
