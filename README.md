...
The book also lives as a browsable [Iris](https://github.com/cgbarlow/iris) Set on the Iris UAT instance:

- **Live**: https://iris-uat.chrisbarlow.nz → *DoView Strategy Models* → *DoView Book*

Every chapter is a Text (Markdown) view; intra-corpus links are rewritten to the `iris://diagram/<id>` scheme so navigation works inside Iris. Mermaid blocks render as SVG (Iris v5.7.0+). The deploy is one-way (disk → Iris) and run from [`scripts/deploy_to_iris_uat.py`](scripts/deploy_to_iris_uat.py) — see [ADR-013](docs/adrs/ADR-013-Iris-Deployment-Script.md) and [`scripts/README.md`](scripts/README.md).
